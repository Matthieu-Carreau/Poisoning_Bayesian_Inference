...
class HorseshoeLinearRegression(ProbabilisticModel):
    """
    The Horseshoe prior for a linear regression model with unknown
    variance and unknown coefficients. Specifically,
        τ ~ C+(1), C+ : Half Cauchy distribution      (global shrinkage)
        λ_j ~ C+(1), 1 <= j <= d                      (local shrinkage)
        β_j | τ, λ_j ~ N(0, τ² λ_j²), 1 <= j <= d     (regression coefficient)
        α ~ N(0, (σ_α)²)                              (intercept)
        σ ~ C+(1)                                     (noise level)
    
    Data likelihood:
        y_i | α, β, X_i, σ ~ N(α + β^T X_i, σ²), 1 <= i <= n

    The implementation is made with numpyro.

    Observed data dictionary format:
        {
        "X": torch.Tensor, shape: (n_data_samples, dim), features matrix.
        "y": torch.Tensor, shape: (n_data_samples, ), vector of output values.
        }
        
    Model parameters dictionary format:
        {
        "local_shrinkage": torch.Tensor, shape: (n_param_samples, dim), λ_j samples.
        "global_shrinkage": torch.Tensor, shape: (n_param_samples, ), τ samples.
        "beta": torch.Tensor, shape: (n_param_samples, dim), β_j samples.
        "intercept": torch.Tensor, shape: (n_param_samples, ), α samples.
        "sigma": torch.Tensor, shape: (n_param_samples, ), Noise level samples.
        }
    """
    
    def __init__(self, intercept_prior_std=100.0, seed=0):
        """
        Initialize the model with Horseshoe prior.

        Parameters
        ----------
        intercept_prior_std : float, optional
            Standard deviation for the gaussian prior for the intercept.
            Default is 100.0 .
        ...
        def model(X, y, weights):
            ...
            intercept = numpyro.sample('intercept', dist.Normal(0, intercept_prior_std))
        ...
        # Initial parameters for Variational Inference approximation of the posterior
    ...
    def sample_posterior(self, n_param_samples: int, data: dict, weights=None,
                         initial_params=None, warmup_steps=100) -> dict:
        ...
        initial_params : dict, optional
            ...
            Dictionary format:
                {
                "local_shrinkage": torch.Tensor, shape: (dim, ), λ_j samples.
                "global_shrinkage": torch.Tensor, shape: (, ), τ samples.
                "beta": torch.Tensor, shape: (dim, ), β_j samples.
                "intercept": torch.Tensor, shape: (, ), α samples.
                "sigma": torch.Tensor, shape: (, ), Noise level samples.
                }
            Default is None.
        ...
        return {k: torch.tensor(np.array(v)) for k, v in mcmc.get_samples().items()} 
